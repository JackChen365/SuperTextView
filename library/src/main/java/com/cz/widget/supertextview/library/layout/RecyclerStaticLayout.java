...
public class RecyclerStaticLayout extends Layout {
    ...
    int fillLineFromStart(Paragraph paragraph, int paragraphLine){
        ...
        int consumed=getLineHeight(off);
        //检测不是结束行,不计算高度消费
        lineCount++;
        return consumed;
    }
    ...
    int fillLineFromEnd(Paragraph paragraph, int paragraphLine){
        ...
        int consumed=getLineHeight(line);
        lineCount++;
        return consumed;
    }
    ...
    public void setMeasureSpecs(int wSpec, int hSpec) {
        textLayoutAdapter.setMeasureSpecs(wSpec,hSpec);
    }
            ...
            this.paragraphArray = new LruCache<Integer,Paragraph>(maxCacheSize){
                ...
                protected void entryRemoved(boolean evicted, Integer key, Paragraph oldValue, Paragraph newValue) {
                    ...
                    for(int i=0;i<oldValue.lineCount;i++){
                        TextElement textElement = oldValue.textLines[i];
